...
#pragma warning disable CS0420 // volatile ref bypass ok for Volatile.Read
            ...
            while (availableSlots > 0)
            {
                ...
                var nodeArray = System.Buffers.ArrayPool<LinkedListNode<KcpSendReceiveBufferItem>>.Shared.Rent(batchSize);
                ...
                finally
                {
                    ...
                    System.Buffers.ArrayPool<(KcpBuffer Data, byte Fragment)>.Shared.Return(dequeueBufferArray, clearArray: true);
                    System.Buffers.ArrayPool<LinkedListNode<KcpSendReceiveBufferItem>>.Shared.Return(nodeArray, clearArray: true);
                }
...
#pragma warning restore CS0420
            ...
            while (nextSn.HasValue && !TransportClosed)
            {
                ...
                // NOTE (Invariant): The following inner loop holds _sndBufLock for O(snd_wnd) iterations.
                // In a single-threaded async pump, this does not cause contention. If multi-threaded access
                // is ever introduced, this lock duration may become a bottleneck and block concurrent HandleAck.
                lock (_sndBufLock)
                {
                    unacknowledged = _rcv_nxt;
    ...
    private async ValueTask UpdateCoreAsync(CancellationToken cancellationToken, uint current)
    {
    ...
    private LinkedListNode<KcpSendReceiveBufferItem>? _lastRcvNodeHint;
        ...
        lock (_rcvBufLock)
        {
            ...
            if (node is not null)
            {
                ...
                if (initialDiff > 0)
                {
                    ...
                    {
                        int diff = TimeDiff(serialNumber, node.ValueRef.Segment.SerialNumber);
                        if (diff < 0) break; // We should insert before the next node, which means `node` is the one to insert after
            ...
            else if (originalBuffer is not null)
            {
                if (originalBuffer is IRefCountedBuffer refCounted)
                {
                    ...
                    try
                    {
                        ...
                        kcpData = KcpBuffer.FromRetainedOwner(refCounted, slice, data.Length);
                    }
                    catch
                    {
                        refCounted.Dispose();
                ...
                {
                    // For unknown IMemoryOwner without reference counting, copy to a pooled buffer
                    var buffer = _bufferPool.Rent(new KcpBufferPoolRentOptions(data.Length, false));
                    try
                    {
                        var spanToCopy = originalBuffer.Memory.Span.Slice(dataOffsetInBuffer, data.Length);
                        kcpData = KcpBuffer.CreateFromSpan(buffer, spanToCopy);
                    }
                    catch
                    {
                        buffer.Dispose();
            ...
            while (node is not null)
            {
                ...
                {
                    if (_lastRcvNodeHint == node)
                        _lastRcvNodeHint = next;
    ...
    public void SetTransportClosed()
    {
        ...
        lock (_rcvBufLock)
        {
            while (_rcvBuf.First is not null)
            {
    ...
    public void Dispose()
    {
        ...
        try { _updateLoopTask?.Wait(2000); } catch { }
