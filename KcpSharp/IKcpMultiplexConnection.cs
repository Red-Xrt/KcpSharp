...
/// </summary>
internal interface IKcpMultiplexConnection : IDisposable
{
    /// <summary>
    ...
    KcpRawChannel CreateRawChannel(uint id, IPEndPoint remoteEndpoint, KcpRawChannelOptions? options = null);
    ...
    /// <exception cref="ObjectDisposedException">The current instance is disposed.</exception>
    /// <exception cref="InvalidOperationException">Another channel or conversation with the same ID was already registered.</exception>
    KcpConversation CreateConversation(uint id, IPEndPoint remoteEndpoint, KcpConversationOptions? options = null);
    ...
    /// <exception cref="ObjectDisposedException">The current instance is disposed.</exception>
    /// <exception cref="InvalidOperationException">Another channel or conversation with the same ID was already registered.</exception>
    void RegisterConversation(IKcpConversation conversation, uint id);
    ...
    IKcpConversation? UnregisterConversation(uint id);
}