...
using LinkedListOfQueueItem =
    System.Collections.Generic.LinkedList<(HyacineCore.Server.Kcp.KcpSharp.KcpBuffer Data, byte Fragment)>;
#endif
...
internal sealed class KcpSendQueue : IValueTaskSource<bool>, IValueTaskSource, IDisposable
{
    ...
    {
        if (_activeWait && !_signaled && _operationMode == 0)
        {
            var buffer = _buffer;
            var mss = _mss;
            var count = buffer.Length <= mss ? 1 : (buffer.Length + mss - 1) / mss;
    ...
    {
        int intBytes = bytes <= int.MaxValue ? (int)bytes : int.MaxValue;
        var unflushedBytes = Interlocked.Add(ref _unflushedBytes, -intBytes);
        if (unflushedBytes <= 0)
        {
            if (unflushedBytes < 0)
            {
