...
/// </summary>
internal interface IKcpMultiplexConnection<T> : IKcpMultiplexConnection
{
    /// <summary>
    ...
    KcpRawChannel CreateRawChannel(uint id, IPEndPoint remoteEndpoint, T state, KcpRawChannelOptions? options = null);
    ...
    /// <exception cref="ObjectDisposedException">The current instance is disposed.</exception>
    /// <exception cref="InvalidOperationException">Another channel or conversation with the same ID was already registered.</exception>
    KcpConversation CreateConversation(uint id, IPEndPoint remoteEndpoint, T state,
    ...
    /// <exception cref="ObjectDisposedException">The current instance is disposed.</exception>
    /// <exception cref="InvalidOperationException">Another channel or conversation with the same ID was already registered.</exception>
    void RegisterConversation(IKcpConversation conversation, uint id, T? state);
    ...
    ///     is not found.
    /// </returns>
    IKcpConversation? UnregisterConversation(uint id, out T? state);
}