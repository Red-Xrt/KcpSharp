...
/// </summary>
internal sealed class KcpRawChannel : IKcpConversation, IKcpExceptionProducer<KcpRawChannel>, IKcpPacketSink
{
    ...
    private readonly System.Threading.Channels.Channel<int> _sendNotification;
    ...
    /// <summary>
    ///     Construct an unreliable channel with a conversation ID.
    /// </summary>
    /// <param name="remoteEndPoint">The remote Endpoint</param>
    /// <param name="transport">The underlying transport.</param>
    /// <param name="conversationId">The conversation ID.</param>
    /// <param name="options">The options of the <see cref="KcpRawChannel" />.</param>
    public KcpRawChannel(IPEndPoint remoteEndPoint, IKcpTransport transport, uint? conversationId,
        KcpRawChannelOptions? options)
    {
        ...
        if ((uint)(_preBufferSize + _postBufferSize) >= (uint)_mtu)
            throw new ArgumentException("The sum of PreBufferSize and PostBufferSize must be less than MTU.",
                nameof(options));
        if (conversationId.HasValue && (uint)(_preBufferSize + _postBufferSize) >= (uint)(_mtu - KcpGlobalVars.CONVID_LENGTH))
        ...
        _sendNotification = System.Threading.Channels.Channel.CreateBounded<int>(
    ...
    public void SetTransportClosed()
    {
        ...
        _sendNotification.Writer.TryWrite(0);
    }
    ...
    public ValueTask<bool> SendAsync(ReadOnlyMemory<byte> buffer, CancellationToken cancellationToken = default)
    {
        ...
        if (_id.HasValue) mss -= KcpGlobalVars.CONVID_LENGTH;
        if (buffer.Length > mss)
            return new ValueTask<bool>(Task.FromException<bool>(
                new ArgumentException($"Message size {buffer.Length} exceeds MSS {mss}.", nameof(buffer))));
    ...
    private async Task RunSendLoopAsync()
    {
        ...
        try
        {
            ...
            {
                var payloadSize = await ev.Reader.ReadAsync(cancellationToken).ConfigureAwait(false);
                ...
                if (_id.HasValue) overhead += KcpGlobalVars.CONVID_LENGTH;
                {
