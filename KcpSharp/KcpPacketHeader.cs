...
{
    internal KcpPacketHeader(KcpCommand command, byte fragment, ushort windowSize, uint timestamp, uint serialNumber,
        uint unacknowledged)
    {
        Command = command;
        Fragment = fragment;
    ...
    internal KcpCommand Command { get; }
    internal byte Fragment { get; }
    internal ushort WindowSize { get; }
    internal uint Timestamp { get; }
    internal uint SerialNumber { get; }
    internal uint Unacknowledged { get; }
    ...
    internal static KcpPacketHeader Parse(ReadOnlySpan<byte> buffer)
    {
    ...
    {
        Debug.Assert(conversationId.HasValue
            ? destination.Length >= KcpGlobalVars.HEADER_LENGTH_WITH_CONVID
            : destination.Length >= KcpGlobalVars.HEADER_LENGTH_WITHOUT_CONVID);
